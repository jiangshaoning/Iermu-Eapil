...
static int player_prepare(PLAYER *player)
{
	...
	//av_dict_set(&opts, "max_analyze_duration", "0", 0);
	//av_dict_set(&opts, "flush-packets", "1", 0);
    ...
    player->render = render_open(
		...
		player->init_params.video_owidth, player->init_params.video_oheight, &(player->init_params.low_delay), player->init_params.eapil_template);
